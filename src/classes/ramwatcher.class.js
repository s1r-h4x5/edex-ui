class RAMwatcher {
    ...
    updateInfo() {
        ...
        window.si.mem().then(data => {
            ...
            const active = Math.round((440 * data.active) / data.total);
            const available = Math.round((440 * (data.available - data.free)) / data.total);
            ...
            // Update info text
            const totalGiB = Math.round((data.total / 1073742000) * 10) / 10; // 1073742000 bytes = 1 Gibibyte (GiB), the *10 is to round to .1 decimal
            const usedGiB = Math.round((data.active / 1073742000) * 10) / 10;
            document.getElementById("mod_ramwatcher_info").innerText = `USING ${usedGiB} OUT OF ${totalGiB} GiB`;

            // Update swap indicator
            const usedSwap = Math.round((100 * data.swapused) / data.swaptotal);
            document.getElementById("mod_ramwatcher_swapbar").value = usedSwap || 0;

            const usedSwapGiB = Math.round((data.swapused / 1073742000) * 10) / 10;
